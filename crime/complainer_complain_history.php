...
<?php
    ...
    if(isset($_POST['s2'])){
        if($_SERVER["REQUEST_METHOD"]=="POST"){
            $cid=$_POST['cid'];
            $_SESSION['cid']=$cid;
            $sql=mysqli_query($conn,"SELECT a_no FROM complaint where c_id='$cid'");
            $qn=mysqli_fetch_assoc($sql);
            if($qn['a_no']==$q2['a_no']){
    ...
    $query="select c_id,type_crime,d_o_c,location from complaint where a_no='$a_no' order by c_id asc";
